...
# fit model
lm2 <- lm(maratime ~ hmaratime * gender + athRank, data = pb_wide)
anova(lm2)
#summary(lm2)
#plot(lm2)
...
Using this model, my marathon prediction is `r toHMS(myprediction$fit - myprediction$se.fit)` to `r toHMS(myprediction$fit + myprediction$se.fit)`, which falls on the slow end of the range from the pace slow-down approach above. This also supports the conclusion that current marathon performance predictors produce overly optimistic predictions for runners.
